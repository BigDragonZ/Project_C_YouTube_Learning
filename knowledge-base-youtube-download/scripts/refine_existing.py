...
1. The pipeline was interrupted mid-run (e.g., Gemini API timeout)
2. A video was processed with subtitle download but refinement failed
3. Manual recovery is needed for individual files

The script:
1. Finds the raw file by index in flow/<course>/
2. Extracts the subtitle/transcription body
3. Calls refine_markdown() with content-preserving prompt
4. Saves as a new refined file
5. Preserves original for safety (manual replacement required)
"""
...
"""
    ...
    print(f"[INFO] To replace: mv \"{refined_path.name}\" \"{file_path.name}\"")
